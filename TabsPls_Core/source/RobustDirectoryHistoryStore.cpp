...
RobustDirectoryHistoryStore& RobustDirectoryHistoryStore::operator=(
    RobustDirectoryHistoryStore other) noexcept {
    swap(*this, other);
    return *this;
}

void RobustDirectoryHistoryStore::OnNewDirectory(
...
void RobustDirectoryHistoryStore::SwitchToPrevious() {
    ...
    const auto previousExistingDir =
        FileSystem::Directory::FromPath(GetCurrent().path());
...
void swap(RobustDirectoryHistoryStore& first,
          RobustDirectoryHistoryStore& second) noexcept {
    using std::swap;
