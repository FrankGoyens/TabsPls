...
TEST_F(FakeFileSystemTest, Separator) { FileSystem::Separator(); }

TEST_F(FakeFileSystemTest, NoFileInEmptyFS) {
    EXPECT_FALSE(FileSystem::IsRegularFile(
        FakeFileSystem::MergeUsingSeparator({"C:", "file.txt"})));
}

TEST_F(FakeFileSystemTest, NoDirectoryInEmptyFS) {
    EXPECT_FALSE(
        FileSystem::IsDirectory(FakeFileSystem::MergeUsingSeparator({"C:"})));
}
...
TEST_F(FakeFileSystemTest, AddFile) {
    ...
    EXPECT_TRUE(FileSystem::IsRegularFile(FakeFileSystem::MergeUsingSeparator(
        {"C:", "users", "jeff", "file.txt"})));
}
...
TEST_F(FakeFileSystemTest, CreateDirectory) {
    ...
    const auto createdDirectory = FileSystem::Directory::FromPath(
        FakeFileSystem::MergeUsingSeparator({"C:", "users", "jeff"}));
    EXPECT_TRUE(createdDirectory.has_value());
    EXPECT_EQ(createdDirectory->path(),
              FakeFileSystem::MergeUsingSeparator({"C:", "users", "jeff"}));
}

TEST_F(FakeFileSystemTest, CreateFilePath) {
    FakeFileSystem::AddFile({"C:", "users", "jeff"}, "file.txt");
    const auto createdFilePath =
        FileSystem::FilePath::FromPath(FakeFileSystem::MergeUsingSeparator(
            {"C:", "users", "jeff", "file.txt"}));
    EXPECT_TRUE(createdFilePath.has_value());
    EXPECT_EQ(createdFilePath->path(),
...
TEST_F(FakeFileSystemTest, RemovedFileName) {
    ...
    const auto createdFilePath =
        FileSystem::FilePath::FromPath(FakeFileSystem::MergeUsingSeparator(
            {"C:", "users", "jeff", "file.txt"}));
    EXPECT_EQ(FileSystem::RemoveFilename(*createdFilePath),
              FakeFileSystem::MergeUsingSeparator({"C:", "users", "jeff"}));
}

TEST_F(FakeFileSystemTest, _getRootPath) {
    EXPECT_EQ(FileSystem::_getRootPath(FakeFileSystem::MergeUsingSeparator(
                  {"C:", "users", "jeff", "file.txt"})),
              std::string("C:") + FileSystem::Separator());
}

TEST_F(FakeFileSystemTest, _getRootName) {
    EXPECT_EQ(FileSystem::_getRootName(FakeFileSystem::MergeUsingSeparator(
                  {"C:", "users", "jeff", "file.txt"})),
              "C:");
}
...
TEST_F(FakeFileSystemTest, GetFilesInDirectory) {
    ...
    const std::vector<FileSystem::Name> expectedContents = {
        "file1.txt", "file2.txt", "file3.txt",
        "file4.txt", "documents", "photos"};
    EXPECT_TRUE(std::is_permutation(
        expectedContents.begin(), expectedContents.end(),
...
TEST_F(FakeFileSystemTest, GetParent) {
    ...
    const auto createdDirectory = FileSystem::Directory::FromPath(
        FakeFileSystem::MergeUsingSeparator({"C:", "users", "jeff"}));
    EXPECT_EQ(FileSystem::GetParent(*createdDirectory),
              FakeFileSystem::MergeUsingSeparator({"C:", "users"}));

    const auto createdRootDirectory = FileSystem::Directory::FromPath(
        FakeFileSystem::MergeUsingSeparator({"C:"}));
    EXPECT_EQ(FileSystem::GetParent(*createdRootDirectory),
              FakeFileSystem::MergeUsingSeparator({"C:"}));
}
...
TEST_F(FakeFileSystemTest, DeleteFile) {
    EXPECT_FALSE(FileSystem::IsRegularFile(FakeFileSystem::MergeUsingSeparator(
        {"C:", "users", "jeff", "file.txt"})));
    FileSystem::Op::RemoveAll(FakeFileSystem::MergeUsingSeparator(
        {"C:", "users", "jeff", "file.txt"}));
