...
TEST_F(FakeFileSystemTest, IsDirectoryEmptyString) { EXPECT_FALSE(FileSystem::IsDirectory("")); }
...
TEST_F(FakeFileSystemTest, IsRegularFileEmptyString) { EXPECT_FALSE(FileSystem::IsRegularFile("")); }
...
TEST_F(FakeFileSystemTest, CreateFilePath) {
    ...
    const auto createdFilePath =
        FileSystem::FilePath::FromPath(FakeFileSystem::MergeUsingSeparator({"C:", "users", "jeff", "file.txt"}));
