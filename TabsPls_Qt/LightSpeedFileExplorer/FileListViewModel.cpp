...
static std::pair<std::vector<FileSystem::FilePath>,
                 std::vector<FileSystem::Directory>>
...
FileListViewModel::FileListViewModel(QStyle& styleProvider,
                                     const QString& initialDirectory)
    : m_styleProvider(styleProvider) {
    std::tie(m_fileEntries, m_directoryEntries) =
        RetrieveDirectoryContents(initialDirectory);
...
bool FileListViewModel::setData(const QModelIndex& index, const QVariant& value,
                                int) {
    const auto fullPathAtIndex =
        ToRawPath(data(index, Qt::UserRole).toString());

    std::optional<std::pair<FileSystem::RawPath, FileSystem::RawPath>>
        renameCall = {};
    ...
    if (const auto dir = FileSystem::Directory::FromPath(fullPathAtIndex)) {
        parentPath = FileSystem::Algorithm::StripTrailingPathSeparators(
            dir->Parent().path());
        if (FileSystem::Algorithm::StripTrailingPathSeparators(dir->path()) ==
            parentPath)
            return false;

        renameCall = std::make_pair(
            dir->path(), parentPath + FileSystem::Separator() +
                             FileSystem::Algorithm::StripLeadingPathSeparators(
                                 ToRawPath(value.toString())));
    } else if (const auto file =
                   FileSystem::FilePath::FromPath(fullPathAtIndex)) {
        parentPath = FileSystem::Algorithm::StripTrailingPathSeparators(
...
static auto CombineAllEntriesIntoNameVec(const FileContainer& files,
                                         const DirContainer& dirs) {
    std::vector<QString> names;
    std::transform(begin(files), end(files), std::back_inserter(names),
                   [&](const auto& file) {
                       return FromRawPath(FileSystem::GetFilename(file));
                   });
    std::transform(begin(dirs), end(dirs), std::back_inserter(names),
                   [&](const auto& dir) {
                       return FromRawPath(FileSystem::GetDirectoryname(dir));
                   });
    return names;
}

template <typename FileContainer, typename DirContainer>
static auto CombineAllEntriesIntoFullPathsVec(const FileContainer& files,
                                              const DirContainer& dirs) {
...
void FileListViewModel::FillModelDataCheckingForRoot(const QString& dir) {
    ...
    if (!DirIsRoot(dir)) {
        ...
        m_icons.insert(m_icons.begin(),
                       m_styleProvider.standardIcon(QStyle::SP_DirIcon));
    }
}
...
void FileListViewModel::FillModelData() {
    ...
    m_fullPaths =
        CombineAllEntriesIntoFullPathsVec(m_fileEntries, m_directoryEntries);
    ...
    std::transform(m_fullPaths.begin(), m_fullPaths.end(),
                   std::back_inserter(m_icons), [&](const auto& fullPath) {
...
QVariant FileListViewModel::headerData(int section, Qt::Orientation,
                                       int role) const {
